...
1. **No Explicit Newlines**: Do not use `\n` or line breaks in the final output string.
2. **Paragraph Separation**: Use natural spacing or punctuation to separate points instead of bullet lists with line breaks.
...
3. **Formatting**: The agent formats each translation to remove newlines.
...
When updating "What's New" text:

```bash
asc app-info set --app <APP_ID> --version-id <VERSION_ID> --locale <LOCALE> --whats-new "<TRANSLATED_TEXT_WITHOUT_NEWLINES>"
```
